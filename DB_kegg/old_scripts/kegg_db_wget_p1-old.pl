...
while(<IN>){
	...
	s/^ko\://;
	my$ko_ID = (split /\t/,$_)[0];
	my$ko_ID_info = get("http://rest.kegg.jp/get/$ko_ID");
	while(defined($ko_ID_info) == 0){ #check point, kegg api revokes long-time user temporarily
		$ko_ID_info = get("http://rest.kegg.jp/get/$ko_ID");
	}
...
close OU4;
close IN1;
print "finished, please run kegg_pku_wget.pl if needed\n";